...
// =============================================================
// MEDICLE — Endless progressive clue-based diagnosis game
//
// ✅ Fixes included:
// 1) Dropdown dedupe + canonical labels (DVT vs deep vein thrombosis → one choice)
// 2) More realistic answer pool (not limited to case answers)
// 3) System normalization (infectious disease vs infectious_disease vs infectious-disease)
// 4) Optional system filter (checkboxes) that keeps session within selected systems
// 5) Multi-file case loading via manifest (drop new files into /public + rebuild)
//
// ✅ How to add more case files without editing the master file:
// - Put files in /public named like: medicle_cases_vol_1.txt, medicle_cases_vol_2.txt, etc.
// - Create /public/medicle_cases_manifest.json containing:
//   { "files": ["/medicle_cases_vol_1.txt", "/medicle_cases_vol_2.txt"] }
// - The page loads the manifest and merges all those files.
// - If the manifest is missing, the page falls back to /cases_master_250.txt.
// =============================================================
...
// ✅ Fallback single-file cases path
const CASES_PATH = "/cases_master_250.txt";

// ✅ Recommended: multi-file cases manifest
// Create this file in /public:
//   /public/medicle_cases_manifest.json
// Example content:
//   { "files": ["/medicle_cases_vol_1.txt", "/medicle_cases_vol_2.txt"] }
const MEDICLE_CASES_MANIFEST_PATH = "/medicle_cases_manifest.json";

// ✅ Optional: large Step 1 diagnosis bank (one diagnosis per line)
...
// Keys MUST be normalizeKeyForDedup(...)
...
export default function Home() {
  ...
  // =============================================================
  // LOAD CASES (manifest multi-file + fallback)
  // =============================================================
  ...
  useEffect(() => {
    ...
    async function loadCases() {
      try {
        // 1) Manifest first
        const manifestRes = await fetch(MEDICLE_CASES_MANIFEST_PATH);
  ...
  // =============================================================
  // DROPDOWN ANSWER BANK (dedupe + expanded pool)
  // =============================================================
  ...
  // =============================================================
  // SUBMIT GUESS (canonical equivalence)
  // =============================================================
      ...
      <main className="min-h-screen flex items-center justify-center px-4" style={{ background: "#022129" }}>
        <div className="max-w-xl rounded-2xl p-6 border" style={{ background: "#0a2f38", borderColor: "#0e3d4a" }}>
          ...
          <p className="text-sm mt-3" style={{ color: "#94a3b8" }}>
            ...
            OR create <span className="font-mono">medicle_cases_manifest.json</span> in public to load multiple files.
          </p>
        </div>
      </main>
    );
  ...
  return (
    <main className="min-h-screen flex flex-col items-center px-4 pb-16" style={{ background: "#022129" }}>
      ...
      {/* GAME OVER MODAL */}
      {gameOver && current && (
        <ResultModal won={won} current={current} guesses={guesses} solvedAtClueCount={solvedAtClueCount} onNext={startNextCase} />
      )}
